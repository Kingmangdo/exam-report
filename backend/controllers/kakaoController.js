...
export const sendSupplementaryNotification = async (req, res) => {
  try {
    ...
    for (const p of participants) {
      ...
      const message = `[독강영어 플러스 케어 타임 안내]
안녕하세요! 꼼꼼한 관리, 독강영어 입니다.
정규 수업에서 배운 내용을 학생의 실력으로 확실히 정착시키기 위해 운영되는 [플러스 케어 타임] 일정을 안내드립니다.
본 세션은 학생마다 다른 학습 성취도와 속도를 세밀하게 점검하여, 다음 단계로 흔들림 없이 나아가기 위해 제공되는 독강영어만의 필수 관리 시간입니다.


▶ 플러스 케어 진행 정보 
◦ 대상 학생: ${studentName} 
◦ 배정 일시: ${formattedDateTime}

▶ 집중 관리 내용 (해당 항목 진행) 
☐ 성취도 보완: 미이수 테스트 및 단어 재시험 완결 
☐ 시험대비 강화: 내신 대비 고난도 문제풀이 및 취약 유형 정밀 지도 
☐ 학습 진도 밸런싱: 정규 과정과의 학습 속도를 맞추기 위한 핵심 포인트 정리 및 연계 가이드


(별) 원장님 안내 말씀 
플러스 케어 타임은 학생의 학습 효율을 극대화하기 위해 해당 시간, 선생님들의 지도 타임을 미리 확보해 두는 방식으로 운영됩니다.
학습의 연속성을 완성하는 귀한 시간인 만큼, 배정된 일정에 차질 없이 참석하여 학생이 자신감 있게 다음 학습을 이어갈 수 있도록 가정에서도 지도 부탁드립니다.`;

      const aligoData = {
        receiver_1: receiverPhone,
        subject_1: '플러스 케어 타임 안내',
        message_1: message,
...
⭐소요 시간: 약 50분
⭐장소: 헤리움리버테라스(죠스떡볶이 건물 2층) 독강학원 데스크로 오시면 안내해드리겠습니다.

⭐ 학습진단은 학생과 먼저 진행되며,진단 종료 시간에 맞춰 학부모님께서 방문해주시면 됩니다.
※ 학생만 방문하는 경우에는 미리 학원으로 알려주시기 바랍니다.
(학부모님의 방문이 어려우실 경우 전화 상담도 가능합니다.)

⭐ 입학 학습진단 비용은 20,000원이며, 등록 시 전액 수강료에서 차감됩니다. (※ 네이버 알림 쿠폰 적용 시 10,000원)

모든 상담은 첼시 원장이 직접 진행합니다. 수업 일정에 따라 상담 시간이 일부 조율될 수 있으며, 조정이 필요한 경우 학원에서 사전에 연락드리겠습니다.

원활한 진행을 위해 예약 시간 5분 전까지 도착 부탁드립니다.

소중한 시간 내어 방문해주셔서 감사합니다. 학생의 현재 상태를 정확히 확인하고, 이후 학습 방향을 차분히 안내드리겠습니다.`;
