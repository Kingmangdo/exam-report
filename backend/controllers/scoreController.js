...
export const getAllScores = (req, res) => {
  try {
    ...
    const scores = Score.getAll(filters);
    res.json({
      success: true,
      data: scores,
      count: scores.length
...
// 성적 ID로 조회
export const getScoreById = (req, res) => {
  try {
    const { id } = req.params;
    const score = Score.getById(id);
...
export const createScore = (req, res) => {
  try {
    ...
    const existing = Score.getByStudentAndDate(student_id, exam_date, class_name);
...
export const updateScore = (req, res) => {
  try {
    ...
    const existingScore = Score.getById(id);
    if (!existingScore) {
      return res.status(404).json({
        success: false,
        message: '성적을 찾을 수 없습니다.'
