...
export const generateReportLink = async (req, res) => {
  try {
    ...
    // 링크 생성
    const token = await Report.createAccessLink(
      score_id,
      student.name,
      phoneLast4
    );
...
export const verifyReportAccess = async (req, res) => {
  try {
    ...
    // 인증 확인
    const verification = await Report.verifyAccess(token, student_name, phone_last4);
...
export const getReportData = async (req, res) => {
  try {
    ...
    // 성적표 데이터 조회
    const reportData = await Report.getReportData(verification.access.score_id);
...
export const previewReport = async (req, res) => {
  try {
    ...
    const reportData = await Report.getPreview(scoreIdNum);
...
// 성적표 링크 상태 확인
export const checkReportLink = async (req, res) => {
  try {
    const { token } = req.params;

    const access = await Report.getAccessByToken(token);
