...
// POST /api/kakao/send-bimonthly - 바이먼스리 알림톡 발송
router.post('/send-bimonthly', sendBimonthlyReport);

// POST /api/kakao/bimonthly-link - 바이먼스리 성적표 링크 생성
router.post('/bimonthly-link', generateBimonthlyReportLink);

// GET /api/kakao/bimonthly-preview/:scoreId - 바이먼스리 성적표 미리보기
router.get('/bimonthly-preview/:scoreId', previewBimonthlyReport);

// POST /api/kakao/bimonthly-verify/:token - 바이먼스리 성적표 인증
router.post('/bimonthly-verify/:token', verifyBimonthlyReportAccess);

// GET /api/kakao/bimonthly-report/:token - 바이먼스리 성적표 공개 조회
router.get('/bimonthly-report/:token', getBimonthlyReportData);

// GET /api/kakao/bimonthly-send-status - 바이먼스리 발송 상태 조회
